...
class MyModule(Module, AutoCSR):
    def __init__(self):
        # CPU から書き込むための CSR (32ビットのストレージ)
        self.data_in  = CSRStorage(32)
        # CPU が読み出すための CSR (32ビットのステータス)
        self.data_out = CSRStatus(32)
        
        # Verilog の MyModule をインスタンス化して接続
        self.specials += Instance("MyModule",
            i_clk         = ClockSignal(),
            i_rst         = ResetSignal(),
            i_cpu_data_in = self.data_in.storage,
            o_cpu_data_out= self.data_out.status
        )
...
class BaseSoC(SoCCore):
    def __init__(self, sys_clk_freq=50e6,
        ...
        **kwargs):
        ...
        # kwargs から不要な引数を削除
        #kwargs.pop("with_ctrl", None)
        kwargs.pop("with_ctrl", True)
        kwargs.pop("integrated_sram_size", None)
        kwargs.pop("csr_paging", None)
        ...
        # ctrl が念のため存在していれば削除
        #if hasattr(self.submodules, "ctrl"):
        #    del self.submodules.ctrl

        # カスタムモジュールの追加
        self.submodules.my_module = MyModule()
        self.add_csr("my_module")  # メモリマップ (CSR) に登録
